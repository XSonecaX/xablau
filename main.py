...
#tanks ferdiz-afk your api
@app.route('/api/lirik', methods=['GET','POST'])
def lirik():
	if request.args.get('judul'):
		try:
			ferdiz = request.args.get('judul')
			url = f'https://ferdiz-api.herokuapp.com/api/lirik?judul={ferdiz}'
			hasil = get(url).json()
			return {
			  'status': 200,
			  'message' : 'Success',
...
@app.route('/api/stalkig/user/name=<username>', methods=['GET','POST'])

def stalkig(username):
    try:
        r = get("https://ferdiz-api.herokuapp.com/api/stalkig/user/name=" + username)
        r = r.json()
        result = {
            "status" : 200,
            "img" : r['image'],
