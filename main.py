...
@app.route('/api/stalkig/user/name', methods=['GET','POST'])
def stalkig():
	if request.args.get('username'):
		try:
			tak = request.args.get('username')
			url = f'https://tahunimek.000webhostapp.com/ig.php?username={tak}'
			r = get(url).json()
     ...
     "bio" : r['bio'],
			...
			return {
				'status': False,
				'error': '[❗] erro talvez om porque o nome de usuário está errado'
			}
	else:
		return {
			'status': False,
			'msg': '[!] Insira o parâmetro de nome de usuário'
		}
